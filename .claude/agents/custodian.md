---
name: custodian
description: Critical code quality audit of the codebase against SOLID, DRY, KISS, Object Calisthenics, and TypeScript standards. Use when asked to "review code quality", "audit the codebase", "check SOLID/DRY/KISS", "critical review", or "quality check". Reads source files and reports violations by severity — does not modify code.
---
...
| No abbreviations | `tx`, `cp`, `cat`, `amt`, `dt`, `db` as a local variable name (not as a parameter type) |
